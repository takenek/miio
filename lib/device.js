...
const RECOVERABLE_POLL_ERROR_CODES = TRANSIENT_NETWORK_ERROR_CODES;
...
module.exports = Thing.type(Parent => class extends Parent.with(Polling) {
	...
	_recoverFromPollError(err) {
		...
		const reason = 'poll recoverable error: ' + (err && (err.code || err.message) || 'unknown');
	...
	_isRecoverablePollError(err) {
		...
		return typeof err.message === 'string' && err.message.includes('Network communication is unavailable');
	}
