...
function isTransientNetworkError(error) {
	...
	return hasTransientOutageMessage(error);
}

function hasTransientOutageMessage(error, seen = new Set()) {
	...
	if (
		...
		error.message.toLowerCase().includes('network communication is unavailable')
	) {
		return true;
	}

	if (error.cause && error.cause !== error) {
		return hasTransientOutageMessage(error.cause, seen);
	}
