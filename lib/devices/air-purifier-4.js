'use strict';

const { AirPurifier } = require('abstract-things/climate');
const MiotApi = require('../iotDevice');

const Power = require('./capabilities/power');
const Mode = require('./capabilities/mode');

const LEDBrightness = require('./capabilities/changeable-led-brightness');
const FanSpeed = require('./capabilities/fan-speed');

const { Temperature, Humidity, AQI } = require('./capabilities/sensor');

/**
 * Abstraction over a Mi Air Purifier.
 *
 * Air Purifiers have a mode that indicates if is on or not. Changing the mode
 * to `idle` will power off the device, all other modes will power on the
 * device.
 */
module.exports = class extends AirPurifier.with(
	MiotApi,
	Power,
	Mode,
	Temperature,
	Humidity,
	AQI,
	LEDBrightness,
	FanSpeed
) {
	static get type() {
		return 'miio:air-purifier';
	}

	constructor(options) {
		super(options);
		const { id } = this.handle.api;

		// Define the power property
		this.defineProperty(
			{ did: `${id}`, siid: 2, piid: 1 },
			{
				name: 'power',
				mapper: v => {
					const { siid, piid } = v;

					if (siid === 2 && piid === 1) {
						return v.value;
					}
				}
			}
		);

		// Set the mode property and supported modes
		this.defineProperty(
			{ did: `${id}`, siid: 2, piid: 4 },
			{
				name: 'mode',
				mapper: v => {
					const { siid, piid } = v;

					if (siid === 2 && piid === 4) {
						return v.value;
					}
				}
			}
		);

		this.defineProperty(
			{ did: `${id}`, siid: 2, piid: 4 },
			{
				name: 'fan',
				mapper: v => {
					const { siid, piid } = v;

					if (siid === 2 && piid === 4) {
						return v.value;
					}
				}
			}
		);

		// Sensor value for Temperature capability
		this.defineProperty(
			{ did: `${id}`, siid: 3, piid: 7 },
			{
				name: 'temperature',
				mapper: v => {
					const { siid, piid } = v;

					if (siid === 3 && piid === 7) {
						return v.value;
					}
				}
			}
		);

		// Sensor value for RelativeHumidity capability
		this.defineProperty(
			{ did: `${id}`, siid: 3, piid: 1 },
			{
				name: 'humidity',
				mapper: v => {
					const { siid, piid } = v;

					if (siid === 3 && piid === 1) {
						return v.value;
					}
				}
			}
		);

		// Sensor value used for AQI (PM2.5) capability
		this.defineProperty(
			{ did: `${id}`, siid: 3, piid: 4 },
			{
				name: 'aqi',
				mapper: v => {
					const { siid, piid } = v;

					if (siid === 3 && piid === 4) {
						return v.value;
					}
				}
			}
		);

		// Info about usage
		// Filter life level
		this.defineProperty(
			{ did: `${id}`, siid: 4, piid: 3 },
			{
				name: 'filter_life',
				mapper: v => {
					const { siid, piid } = v;

					if (siid === 4 && piid === 3) {
						return v.value;
					}
				}
			}
		);

		// State for SwitchableLED capability
		this.defineProperty(
			{ did: `${id}`, siid: 13, piid: 2 },
			{
				name: 'led_brightness',
				mapper: v => {
					const { siid, piid } = v;

					if (siid === 13 && piid === 2) {
						return v.value;
					}
				}
			}
		);

		// Child Lock
		this.defineProperty(
			{ did: `${id}`, siid: 8, piid: 1 },
			{
				name: 'child_lock',
				mapper: v => {
					const { siid, piid } = v;

					if (siid === 8 && piid === 1) {
						return v.value;
					}
				}
			}
		);

		// // Buzzer and beeping
		this.defineProperty(
			{ did: `${id}`, siid: 6, piid: 1 },
			{
				name: 'buzzer',
				mapper: v => {
					const { siid, piid } = v;

					if (siid === 6 && piid === 1) {
						return v.value;
					}
				}
			}
		);

		// Favorite Fan Level
		this.defineProperty(
			{ did: `${id}`, siid: 10, piid: 10 },
			{
				name: 'favorite_level',
				mapper: v => {
					const { siid, piid } = v;

					if (siid === 10 && piid === 10) {
						return v.value;
					}
				}
			}
		);

		// Motor Speed
		this.defineProperty(
			{ did: `${id}`, siid: 9, piid: 1 },
			{
				name: 'fan_speed',
				mapper: v => {
					const { siid, piid } = v;

					if (siid === 9 && piid === 1) {
						return v.value;
					}
				}
			}
		);
	}

	// Child lock
	childLock() {
		return Promise.resolve(this.property('child_lock').value);
	}

	updateChildLock(value) {
		if(this.updateState('child_lock', value)) {
			this.emitEvent('childLockChanged', value);
		}
	}


	/**
	 * Filter information
	 */
	
	 filterLifeLevel() {
		return Promise.resolve(this.property('filter_life').value);
	}

	updateFilterLifeLevel(value) {
		if(this.updateState('filter_life', value)) {
			this.emitEvent('filterLifeChanged', value);
		}
	}

	propertyUpdated(key, value) {
		if(key === 'fan_speed' && value !== undefined) {
			this.updateFanSpeed(value);
		}

		if (key === 'child_lock') {
			this.updateChildLock(value);
		}

		if(key === 'filter_life' && value !== undefined) {
			this.updateFilterLifeLevel(value);
		}

		super.propertyUpdated(key, value);
	}

	changePower(value) {
		const { id } = this.handle.api;
		return this.call('set_properties', [
			{ did: `${id}`, siid: 2, piid: 1, value: !!value }
		]);
	}

	/**
	 * Perform a mode change as requested by `mode(string)` or
	 * `setMode(string)`.
	 */
	changeMode(mode) {
		const { id } = this.handle.api;
		let value;

		switch (mode) {
			case 'auto':
				value = 0;
				break;

			case 'sleep':
				value = 1;
				break;

			case 'favorite':
				value = 2;
				break;

			case 'none':
				value = 3;
				break;
		}

		return this.call('set_properties', [
			{ did: `${id}`, siid: 2, piid: 4, value }
		]);
	}

	changeFan(speed) {
		const { id } = this.handle.api;
		let value;

		value = parseInt(speed);

		if (value < 1) {
			value = 1;
		}
		if (value > 14) {
			value = 14;
		}

		return this.call('set_properties', [
			{ did: `${id}`, siid: 9, piid: 2, value }
		]);
	}

	changeFavoriteSpeed(speed) {
		const { id } = this.handle.api;
		let value = parseInt(speed);

		if (value < 1) {
			value = 1;
		}
		if (value > 14) {
			value = 14;
		}

		return this.call('set_properties', [
			{ did: `${id}`, siid: 9, piid: 2, value }
		]);
	}

	/**
	 * Set the LED brightness to either `bright`, `dim` or `off`.
	 */
	changeLEDBrightness(level) {
		const { id } = this.handle.api;
		let value;

		switch (level) {
			case 'bright':
				value = 0;
				break;
			case 'dim':
				value = 1;
				break;
			case 'off':
				value = 2;
				break;
			default:
				return Promise.reject(new Error('Invalid LED brightness: ' + level));
		}

		return this.call('set_properties', [
			{ did: `${id}`, siid: 6, piid: 1, value }
		]).then(() => null);
	}

	changeChildLock(lock) {
		const { id } = this.handle.api;

		return this.call('set_properties', [
			{ did: `${id}`, siid: 7, piid: 1, value: !!lock }
		]).then(() => null);
	}

	changeBuzzer(value) {
		const { id } = this.handle.api;

		return this.call('set_properties', [
			{ did: `${id}`, siid: 5, piid: 1, value: !!value }
		]).then(() => null);
	}

	changeFavoriteLevel(level) {
		const { id } = this.handle.api;
		let value = parseInt(level);

		if (value < 0) {
			value = 0;
		}

		if (value > 14) {
			value = 14;
		}

		return this.call('set_properties', [
			{ did: `${id}`, siid: 10, piid: 10, value }
		]);
	}

	changeFanSpeed(speed) {
		const { id } = this.handle.api;
		let value;

		value = parseInt(speed);

		if (value < 0) {
			value = 0;
		}
		if (value > 3000) {
			value = 3000;
		}

		return this.call('set_properties', [
			{ did: `${id}`, siid: 10, piid: 8, value }
		]);
	}
};
