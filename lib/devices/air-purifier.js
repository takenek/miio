		...
		SwitchableLED, LEDBrightness, Buzzer, FanSpeed)
{
	...
	constructor(options) {
		...
		// Info about usage
		// https://miot-spec.org/miot-spec-v2/spec/property?type=urn:miot-spec-v2:property:filter-life-level:0000001E
	...
	setVolume(volume){
		return this.call('set_volume', [volume], {
			refresh: ['volume'],
			refreshDelay: 200
		});
	}
	...
	filterDaysUsed(filterDaysUsed = undefined) {
		if (typeof filterDaysUsed === 'undefined') {
			return Promise.resolve(Math.floor(this.property('filterHoursUsed') / 24.0));
		}

		return this.filterDaysUsed(filterDaysUsed);
	}

	filterLifeLevel(filterLifeLevel = undefined) {
		if (typeof filterHoursRemaining === 'undefined') {
			return Promise.resolve(this.property('filterLifeLevel'));
		}

		return this.filterHoursRemaining(filterLifeLevel);
	}
