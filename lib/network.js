...
} = require('./transientNetworkErrors');
...
const RECOVERY_DISCOVERY_DEBOUNCE = 5000;
const RECOVERY_DISCOVERY_RETRY_DELAY = 300;
...
function setUnrefTimeout(handler, delay) {
...
class Network extends EventEmitter {
	constructor() {
		...
		this._lastRecoveryDiscovery = 0;
		this._pendingRecoveryDiscovery = null;
	}

	requestRecoveryDiscovery(reason) {
		if (this.references <= 0) return;

		const now = Date.now();
		if (now - this._lastRecoveryDiscovery < RECOVERY_DISCOVERY_DEBOUNCE) {
			return;
		}

		this._lastRecoveryDiscovery = now;

		if (this._socketResetInProgress || !this._socket) {
			this.debug('Deferring discovery recovery until socket is available', reason || 'unknown reason');
			this._scheduleRecoveryDiscovery(reason);
			return;
		}

		this._runRecoveryDiscovery(reason);
	}

	_scheduleRecoveryDiscovery(reason) {
		if (this._pendingRecoveryDiscovery) return;

		this._pendingRecoveryDiscovery = setUnrefTimeout(() => {
			this._pendingRecoveryDiscovery = null;

			if (this.references <= 0) return;

			this._runRecoveryDiscovery((reason || 'unknown reason') + ' (deferred)');
		}, RECOVERY_DISCOVERY_RETRY_DELAY);
	}

	_runRecoveryDiscovery(reason) {
		this.debug('Triggering discovery to recover from network instability', reason || 'unknown reason');
		try {
			this.search();
		} catch (err) {
			this.debug('Recovery discovery failed synchronously', err);
		}
	}
		...
		const sendBroadcast = () => {
			...
			try {
				...
				if (isTransientError(err)) {
					...
					this.resetSocket('discovery socket unavailable: ' + err.code);
					return;
				}
			...
			try {
				...
				if (isTransientError(err)) {
					this.debug('Discovery broadcast threw due to transient network issue', err.code);
					this.resetSocket('discovery broadcast throw: ' + err.code);
					return;
				}
	...
	createSocket() {
		...
		try {
			...
			if (isTransientError(err)) {
				this.debug('Failed to bind network socket due to transient issue', err.code);
				this.resetSocket('socket bind error: ' + err.code);
				return;
			}
		...
		socket.on('listening', () => {
			try {
				...
				if (isTransientError(err)) {
					this.debug('Failed to finalize network socket startup due to transient issue', err.code);
					this.resetSocket('socket listening setup error: ' + err.code);
					return;
				}

				this.debug('Unexpected error while finalizing network socket startup', err);
			}
		});
	...
	updateSocket() {
		if (this.references === 0) {
			if (this._pendingRecoveryDiscovery) {
				clearTimeout(this._pendingRecoveryDiscovery);
				this._pendingRecoveryDiscovery = null;
			}
...
class DeviceInfo {
	...
	onMessage(msg) {
		...
		if (data === null) {
			...
			try {
				...
				} else if (typeof p.retry === 'function') {
					p.retry({ code: 'invalid-response', message: 'Device replied without result or error' });
				} else {
					p.reject(new Error('Device replied without result or error'));
				}
			} catch (ex) {
	...
	_isRetryableDeviceError(err) {
		...
		return (this.handshakePromise = new Promise((resolve, reject) => {
			...
			this.handshakeTimeout = setUnrefTimeout(() => {
	...
	call(method, args, options) {
		...
		return new Promise((resolve, reject) => {
			...
			const recoverFromNetworkError = reason => {
				...
				this.parent.requestRecoveryDiscovery(reason);
			};
			...
			const promise = {
				...
				reject: err => {
					...
					if (!err) {
						err = new Error('Device returned an invalid response');
						err.code = 'invalid-response';
					} else if (!(err instanceof Error) && typeof err.code !== 'undefined') {
						const code = err.code;

						const handler = ERRORS[code];
						let msg;
						if (handler) {
							msg = handler(method, args, err);
						} else {
			...
			const retry = reason => {
				...
				if (reason === 'call timeout' || reason === 'handshake timeout') {
					recoverFromNetworkError('retry due to ' + reason);
				}
			...
			const send = () => {
				...
				this.handshake()
					...
					.then(token => {
						...
						responseTimeout = setUnrefTimeout(() => {
