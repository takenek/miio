...
const { TimedDiscovery, BasicDiscovery, search, addService, removeService } = require('./discovery-base');
...
const Browser = module.exports.Browser = class Browser extends TimedDiscovery {
	...
	start() {
		if(this._startedNetwork) {
			super.start();
			return;
		}

		this._startedNetwork = true;
	...
	[tryAdd](device) {
		...
		const addFallbackTimer = setTimeout(add, 5000);
		if(typeof addFallbackTimer.unref === 'function') {
			addFallbackTimer.unref();
		}

		dns.lookupService(service.address, service.port, (err, hostname) => {
			clearTimeout(addFallbackTimer);
